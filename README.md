...
| `/lexicon/:term` | Permanent, source-aware term pages with local connections. |
...
| `/sources` | Browsable manuscript-derived source records. |
| `/search` | Global search across terms, aliases, acronyms, definitions, connected concepts, and domains. |
...
This creates `client/public/sitemap.xml` from the seed and includes the public platform routes, twenty domain URLs, and stable Lexicon term URLs. Do not change published term slugs casually; they are intended for citations and future QR codes.
...
The platform applies route-aware titles, descriptions, canonicals, Open Graph data, and client-side structured data. Individual Lexicon entries publish `DefinedTerm` and `BreadcrumbList` schema. The static document head retains `DefinedTermSet`, Book, Product-offer, and Organization schema. The sitemap currently represents the public platform, legal routes, 20 domains, and 500 term URLs.
