...
const editions = editionCatalog.map((edition) => ({
  ...edition,
  link: import.meta.env[edition.linkKey] as string | undefined,
}));

type FormSource = "preorder-form" | "starter-pack-form";
type EditionInterest = "collector" | "hardcover" | "paperback" | "starter-pack" | "updates";

function EditionButton({ edition }: { edition: (typeof editions)[number] }) {
  const handleClick = () => {
    if (!edition.link) {
      toast.info("Checkout link coming soon", { description: "Your Stripe payment link will appear here before the preorder opens." });
      return;
    }
  ...
  return (
    <div className="orbion-site">
      ...
      <main id="top">
        ...
        <section className="trust-path"><div className="section-shell"><div className="trust-intro"><div><p className="eyebrow"><ShieldCheck className="h-4 w-4" /> A CAREFULLY PREPARED FIRST EDITION</p><h2>From campaign<br /><em>to collector.</em></h2></div><p>Preorders will open only when the completed book, production plan, and delivery window are locked. Until then, the list gives you first access to the illustrated preview and release updates.</p></div><div className="trust-steps"><div><span>01</span><strong>Explore</strong><p>Read real illustrated entries.</p></div><div><span>02</span><strong>Join the list</strong><p>Choose your edition interest.</p></div><div><span>03</span><strong>Receive updates</strong><p>Get the locked release window.</p></div><div className="active"><span>04</span><strong>Preorder</strong><p>Checkout opens when ready.</p></div></div></div></section>
