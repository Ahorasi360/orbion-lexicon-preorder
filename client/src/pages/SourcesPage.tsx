...
export default function SourcesPage() {
  ...
  return <PlatformShell>
    ...
    <section className="sources-browser"><div className="platform-container">{isLocked ? <div className="empty-lexicon"><p>Source and evidence records are included with Orbion Online Lexicon access.</p><a href="/lexicon/access">Unlock Online Lexicon access →</a></div> : <><div className="source-search"><Search size={18} aria-hidden="true" /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Filter source ID, title, or publisher" aria-label="Filter source records" /><span>{isLoading ? "Loading…" : `${sources.length} records`}</span></div><div className="source-records">{sources.map((source: SourceRecord) => <article key={source.orbionId} className="source-record"><span>{source.orbionId}</span><div><h2>{source.title}</h2><p>{[source.publisher, source.sourceType.replaceAll("_", " "), source.rightsStatus.replaceAll("_", " ")].filter(Boolean).join(" · ")}</p></div>{source.locator ? <a href={source.locator} target="_blank" rel="noreferrer" onClick={() => trackEvent("source_open", { sourceId: source.orbionId })} aria-label={`Open source ${source.orbionId}`}><ExternalLink size={16} /></a> : <span className="source-record-pending">Review pending</span>}</article>)}</div></>}</div></section>
  </PlatformShell>;
}
