...
const pages: Record<string, PolicyPage> = {
  "terms-of-sale": {
    ...
    sections: [
      ...
      { heading: "Online Lexicon annual access", paragraphs: ["Orbion Online Lexicon access is a separately sold digital product. The annual access offer currently displayed at checkout is a one-time payment for one year of account-linked access. It does not include a physical book, does not renew automatically, and does not alter any physical-book preorder, delivery, cancellation, or refund terms.", "Access is granted only after Stripe payment is verified by Orbion’s server and linked to the signed-in account used to begin checkout. The price displayed when a digital-access purchase is completed applies to that purchase. Orbion may change pricing for future Online Lexicon offers at its discretion."] },
      { heading: "Digital access cancellation and refunds", paragraphs: ["Where digital access is made available immediately after a verified purchase, the purchase is final and non-refundable once access has been granted, except where applicable law requires otherwise. The Preorder, Cancellation & Refund Policy applies to physical-book preorders and does not create a cancellation right for completed digital-access periods. If access cannot be granted because of an Orbion error, contact hello@orbionlexicon.com so Orbion can review an appropriate remedy under applicable law."] },
  ...
  "privacy-policy": {
    ...
    sections: [
      { heading: "Information collected", paragraphs: ["When you join the first-edition list or request the Starter Pack, Orbion collects your name, email address, edition interest, and the source of your signup. If you sign in to use or purchase Online Lexicon access, Orbion also uses the account identity supplied by the sign-in provider and stores access, purchase, and entitlement status needed to deliver the digital service. Payment information is handled by Stripe during checkout and is not collected by the Orbion website form."] },
      { heading: "How information is used", paragraphs: ["Orbion uses your information to deliver the requested Starter Pack, send first-edition and production updates, provide preorder-related communications, respond to support requests, maintain the preorder-interest list, and provide account-linked Online Lexicon access after a verified payment. You may unsubscribe from promotional email or request deletion of eligible list information by contacting hello@orbionlexicon.com."] },
...
export default function LegalPage({ slug }: { slug: keyof typeof pages }) {
  ...
  return <><Seo title={`${page.title} | The Orbion Space Lexicon`} description={page.summary} canonicalPath={`/${slug}`} /><div className="legal-site">
    ...
    <main className="legal-main">
      ...
      <p className="legal-date">Effective date: August 13, 2026 · Questions: <a href={`mailto:${supportEmail}`}>{supportEmail}</a></p>
