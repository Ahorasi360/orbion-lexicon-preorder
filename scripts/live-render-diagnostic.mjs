...
await browser.close();
const result = { url, checkedAt: new Date().toISOString(), consoleErrors, pageErrors, failedRequests, rendered };
await mkdir(path.join(root, "docs"), { recursive: true });
await writeFile(path.join(root, "docs", "live-render-diagnostic.json"), `${JSON.stringify(result, null, 2)}\n`);
console.log(JSON.stringify(result, null, 2));
if (!rendered.rootText.trim() || consoleErrors.length || pageErrors.length || failedRequests.length) process.exit(1);
