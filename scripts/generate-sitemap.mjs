...
const staticRoutes = [
  ...
  ["/maps", "monthly", "0.7"], ["/methodology", "monthly", "0.7"],
...
const termUrls = seed.entries.filter(entry => entry.isPublicPreview === true && entry.indexStatus === "index").map(entry => `  <url><loc>${siteUrl}/lexicon/${escapeXml(entry.slug)}</loc><changefreq>monthly</changefreq><priority>0.7</priority></url>`);
